        ...
        <Menu as="div" className="relative inline-block text-left">
          <div className='flex items-center'>
            <Menu.Button className="group flex items-center rounded-md px-4 py-2 text-sm font-medium text-white hover:bg-opacity-30 focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 md:mr-4">
                <BsPersonCircle className='h-6 w-6 mr-1 text-gray-700 group-hover:text-gray-400 sm:flex' aria-hidden='true'/>
                <BsCaretDownFill className='h-4 w-4 text-gray-700 group-hover:text-gray-400 sm:flex group-aria-pressed:rotate-180' aria-hidden='true'/>
            </Menu.Button>
          </div>
          <Transition
          as={Fragment}
          ...
          {token ? (
            <>
              ...
              <Menu.Item>
                  {({ active }) => (
                  <button
                      onClick={()=>{localStorage.removeItem('AuthToken'); router.reload()}}
                      className={`${
                      active ? 'bg-red-500 text-white' : 'text-gray-900'
                      } group flex w-full items-center rounded-md px-2 py-2 text-sm`}>
                      {active ? (
