...
import { reqdepthistory, reqfindemployee, reqsalaryhistory } from "../../redux/hr-schema/action/actionReducer";
...
const SalaryHistory = (props: any) => {

    const dispatch = useDispatch()

    const { findEmp, salhis, message, refresh } = useSelector((state: any) => state.empReducers)
    ...
    useEffect(() => {
        if (router.isReady) {
            ...
            dispatch(reqsalaryhistory(id))

        }

        console.log("ID", id);
        console.log('dataNamaDepartment', salhis)
    }, [router.isReady]
    )
    ...
    const TABLE_HEAD = ["Date", "Nominal", "Pay Frequence"]
            ...
            <Card className=" h-full w-full">
                ...
                <CardBody>
                    <table className="mt-4 w-full min-w-max table-auto text-left">
                        ...
                        <tbody>
                            {(salhis || []).map((dt: any, index: any) => {
                                const isLast = index === salhis.length - 1;
