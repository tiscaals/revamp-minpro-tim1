...
const JobDetail = () => {
  ...
  const { job_post, refresh: refreshJopo } = useSelector(
    (state: any) => state.JobPostReducers
  );
        ...
        <div>
          ...
          <section className="p-8 border-b-2">
            <div className="container">
              <div className="flex">
                <img
                  src={`http://localhost:3003/image/${job_detail?.jopho_filename}`}
                  alt="gambar"
                  className="float-left mr-3 object-contain w-20 pr-2 md:h-24 md:w-24 lg:h-28 lg:w-28"
                />
                <div className="w-full">
                  ...
                  <div className="flex items-center">
                    ...
                    <h3 className="text-sm font-normal pl-2 md:text-base lg:text-lg">
                      IDR {formattedMinSalary} - {formattedMaxSalary}/month
                    </h3>
                  </div>
                  <div className="flex items-center">
                    <svg
                    ...
                    {job_role.map((option: any) =>
                      job_detail?.jopo_joro_id === option.joro_id ? (
                        <h2
                          key={option.joro_id}
                          className="text-sm underline text-blue-500  font-normal pl-2 md:text-base lg:text-lg"
                        >
                          {option.joro_name}
                        </h2>
                      ) : null
                  ...
                  <div className="grid grid-cols-2 max-w-sm pt-2 lg:pt-4">
                      ...
                      {work_type.map((option: any) =>
                        job_detail?.jopo_work_code === option.woty_code ? (
                          <h2
                            ...
                            className="text-xs font-light pl-2 md:text-sm lg:text-md"
                          >
                            {option.woty_name}
                          </h2>
                        ) : null
                    ...
                    <div className=" flex  items-center">
                      ...
                      <h3 className="text-xs  font-light pl-2 md:text-sm lg:text-md">
                        {job_detail?.jopo_min_experience}-
                        {job_detail?.jopo_max_experience} tahun
                      </h3>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 max-w-sm pt-2 lg:pt-4">
                    <div className=" flex items-center">
                      ...
                      <h3 className="text-xs  font-light pl-2 md:text-sm lg:text-md">
                        {job_detail?.city_name}
                      </h3>
                    </div>
                    <div className=" flex items-center">
                      <ClockIcon height="20" width="20" />
                      <h3 className="text-xs  font-light pl-2 md:text-sm lg:text-md">
                        {job_detail?.jopo_publish_date ? (
                          <>
                            {(() => {
              ...
              <div className="flex flex-wrap">
                <div className="w-full ">
                  ...
                  <div className="flex gap-2 items-center">
                    {job_detail?.jopo_secondary_skill
                      ?.split(',')
                      .map((skill: any) => (
                        <p className="text-base bg-blue-700 text-white rounded-lg px-2 py-1 md:px-3 lg:px-4">
                          {skill.trim()}
              ...
              <div className="flex flex-wrap">
                <div className="w-full border border-slate-500 px-3 py-2 shadow-lg">
                      ...
                      {industry.map((option: any) =>
                        job_detail?.clit_indu_code === option.indu_code ? (
                          <h2
                            ...
                            className="text-sm font-light pb-2 md:text-lg"
                          >
                            {option.indu_name}
                          </h2>
                        ) : null
                  ...
                  <p className="text-sm max-w-xl pb-6 md:text-lg">
                    {job_detail?.clit_about}
                  </p>
                  <h1 className="text-lg md:text-xl lg:text-2xl pb-3">
                    Address
                  </h1>
                  <p className="text-sm max-w-xl pb-6 md:text-lg">
                    {job_detail?.addr_line1},{' '}
                    {job_detail?.addr_line2 ? `${job_detail?.addr_line2},` : ''}{' '}
                    {job_detail?.city_name}
                  </p>
                </div>
              </div>
