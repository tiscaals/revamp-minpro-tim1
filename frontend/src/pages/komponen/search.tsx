...
const SearchBar = (props: any) => {
  ...
  const { job_role, refresh } = useSelector(
      ...
      <div className="relative lg:flex lg:flex-wrap justify-between items-center lg:justify-center mx-auto">
        ...
        <div className="pb-4 lg:pb-0 lg:pl-4">
          <div className="w-auto">
            <select
              className="bg-white p-2.5 border-gray-400 border-[1px] rounded-lg"
              value={selectedValue}
              onChange={handleChange}
            >
              <option value="">All</option>
              {job_role.map((option: any) => (
                <option key={option.joro_id} value={option.joro_name}>
                  {option.joro_name}
                </option>
              ))}
            </select>
        ...
        <div className="pb-2 lg:pb-0 lg:pl-4">
          <Button className="lg:w-52 w-full" onClick={buttonClick}>
            SEARCH
          </Button>
        </div>
      </div>
