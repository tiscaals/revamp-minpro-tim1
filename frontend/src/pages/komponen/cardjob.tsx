...
const CardJob = ({
  ...
  if (filtering) {
    ...
    const filteredItems = itemSimilarityScore.filter(
      (item: any) => item.jopo_entity_id != id
    );
    // console.log("filteredItems", filteredItems);
    job_data = filteredItems.sort(
      (a: any, b: any) => a.similarityScore - b.similarityScore
    );
            ...
            <div className="hover:opacity-70 transition ease-in-out pb-1">
              <div className="w-auto p-5 bg-white border shadow-lg block ">
                <div className="flex items-center">
                  {/*---- Gambar ----*/}
                  <img
                    src={`http://localhost:3003/image/${data.jopho_filename}`}
                    alt="gambar"
                    className="object-contain h-14 w-24 mr-5"
                  />
                  <div className="">
                    {/*---- Title ----*/}
                    <h1 className="text-xl font-semibold">{data.jopo_title}</h1>
                    {/*---- Nama Perusahaan ----*/}
                    <h1 className="text-md font-medium">{data.clit_name}</h1>
                  </div>
                </div>
                <div className="pt-6">
                  <div className="my-2">
                    ...
                    <div className="flex">
                      ...
                      <h3 className="text-sm">
                        {data.jopo_min_experience}-{data.jopo_max_experience}{' '}
                        Tahun
                      </h3>
                    </div>
                  </div>
                  ...
                  <div className="flex justify-between pt-2">
                    <div className="flex rounded-md text-green-500">
                      ...
                      <h3 className="text-sm">Actively Hiring</h3>
                    </div>
                    <div className="flex">
