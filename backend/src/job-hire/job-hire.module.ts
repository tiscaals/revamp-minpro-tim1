import { Module } from '@nestjs/common';
import { JobHireService } from './job-hire.service';
import { JobHireController } from './job-hire.controller';
import { SequelizeModule } from '@nestjs/sequelize';
import { client, job_category, job_photo, job_post, job_post_desc } from 'models/job_hire';

@Module({
  imports: [SequelizeModule.forFeature([job_category, job_photo, job_post, job_post_desc, client])],
  controllers: [JobHireController],
  providers: [JobHireService]
})
export class JobHireModule {}
