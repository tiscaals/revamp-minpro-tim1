...
import { program_entity } from 'models/curriculum';
...
//   create section
  async createSections(createProgramEntity: any): Promise<any> {
    try {
      console.log(createProgramEntity);
      const data = `[${JSON.stringify(createProgramEntity)}]`;
      const query = `CALL curriculum.sections_add('${data}')`;
      const result = await this.sequelize.query(query);
      return { data: result, messages: 'Data Sukses Di Input' };
    } catch (error) {
      return error.message;
...
//   update program
  async update(
    ...
    try {
      ...
      const query = `CALL curriculum.update_program_(${id}, '${paramProgram}')`;
      const result = await this.sequelize.query(query);
